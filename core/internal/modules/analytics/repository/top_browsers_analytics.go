...
func (tb *TopBrowsersAnalytics) GetTopBrowsers(ctx context.Context, websiteID string, days int, timezone string, limit int) ([]models.BrowserStat, error) {
	...
	rows, err := tb.db.Query(ctx, query, websiteID, days, timezone, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
