...
const project = `seentics-heatmap-e2e-${randomUUID().slice(0, 8)}`;
...
const compose = ['compose', '--project-name', project, '--file', path.join(root, 'web/e2e/heatmap/compose.yml')];
let activeChild;
...
try {
  ...
  if (!env.E2E_WEB_IMAGE) {
    const build = await run('docker', [
      'build', '--target', 'web', '--tag', `seentics-heatmap-e2e-web:${project}`,
      '--file', path.join(root, 'web/e2e/heatmap/Dockerfile'), root,
    ]);
    if (build.code !== 0) throw new Error('Dashboard image failed to build.');
    env.E2E_WEB_IMAGE = `seentics-heatmap-e2e-web:${project}`;
    builtImages.push(env.E2E_WEB_IMAGE);
  }
  if (!env.E2E_CORE_IMAGE) {
    const build = await run('docker', [
      'build', '--target', 'core', '--tag', `seentics-heatmap-e2e-core:${project}`,
      '--file', path.join(root, 'web/e2e/heatmap/Dockerfile'), root,
    ]);
    if (build.code !== 0) throw new Error('Core image failed to build.');
    env.E2E_CORE_IMAGE = `seentics-heatmap-e2e-core:${project}`;
  ...
  if (up.code !== 0 || interrupted) throw new Error('Heatmap test stack failed to start. See Docker logs in ' + artifacts);
  const result = await run(process.execPath, [
    path.join(web, 'node_modules/@playwright/test/cli.js'), 'test',
    '--config', 'e2e/heatmap/playwright.config.ts', ...process.argv.slice(2),
  ], { cwd: web });
