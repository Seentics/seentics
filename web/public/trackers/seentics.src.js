...
(function(w, d) {
  ...
  var initSession = function() {
    ...
    S.sid = ss.getItem('_ss'); var la = ss.getItem('_sl');
    if (!S.sid || !la || Date.now() - parseInt(la) > 1800000) { S.sid = uuid(); ss.setItem('_ss', S.sid); }
    ss.setItem('_sl', Date.now() + '');
  };
  ...
  var onNav = function() {
    if (loc.pathname !== _path) { _path = loc.pathname; pageview(); flush(); emit('nav', { path: _path }); }
  };
  ...
  var hmCoords = function(e) {
    var dims = hmDims(), bw = dims.w || 1, dt = hmDev(), x;
    if (dt !== 'mobile') { var tw = dt === 'desktop' ? 1200 : 768; x = ((tw / 2) + (e.pageX - dims.l - bw / 2)) / tw * 1000; }
    else x = ((e.pageX - dims.l) / bw) * 1000;
    return { x: x, y: Math.min(1000, Math.max(0, (e.pageY / (dims.h || 1)) * 1000)) };
  };
  ...
  var hmAdd = function(type, x, y, sel) {
    ...
    if (buf.heatmaps.length >= 100) flush();
  };
  ...
  var initHeatmap = function() {
    ...
    w.addEventListener('message', function(ev) {
      if (ev.data === 'SEENTICS_GET_DIMENSIONS') {
        var dims = hmDims();
        ev.source.postMessage({ type: 'SEENTICS_DIMENSIONS', height: dims.h, width: dims.w,
    ...
    loadRR().then(function() {
      ...
      var stop = w.rrweb.record({
        emit: function(ev) {
          ...
          if ((buf.replay.seq === initSeq && buf.replay.events.length >= 2) || buf.replay.events.length >= 500) flush();
        },
        maskAllInputs: true, maskInputOptions: { password: true, email: true }
      });
      if (!stop) return;
      setTimeout(function() { flush(); stop(); }, 30 * 60 * 1000);
    }).catch(function() {});
  };
