...
/**
 * The two panels of the developers page: embeddable UI blocks and the SDK docs.
 *
 * Both take only the website id, so either can be dropped into a docs route or an
 * onboarding step without the page around it.
 */
...
export function UIBlocksTab({ websiteId }: { websiteId: string }) {
...
export const SDK_TABS = ['Node.js', 'Go'] as const;
export type SdkTab = typeof SDK_TABS[number];

export const INSTALL: Record<SdkTab, string> = {
  'Node.js': `npm install @seentics/node`,
  'Go':      `go get github.com/seentics/go-sdk`,
};
export const QUICKSTART: Record<SdkTab, string> = {
...
export function DocsTab({ websiteId }: { websiteId: string }) {
